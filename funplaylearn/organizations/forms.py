...
class OrganizationForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if any(cleaned_data.get(f) for f in contact_fields):
            ...
            ):  # we are saying if its an existing org with contact we just load that or else we create a new one
...
class SchoolForm(forms.ModelForm):
    ...
    school_palika = forms.ModelChoiceField(
        label="Palika", queryset=Palika.objects.all(), required=True
    )
    # contact info
    ...
    latitude = forms.DecimalField(
        label="latitude", max_digits=9, decimal_places=6, required=True
    )
    longitude = forms.DecimalField(
        label="longitude", max_digits=9, decimal_places=6, required=True
    )
    general_location = forms.CharField(
        label="general_location", max_length=255, required=True
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        )  # this is getting the org attribite from the school model instanceand defaultingto none if not found
        if organization:  # checking if its none
            self.fields["school_name"].initial = (
                organization.name
            )  # pre populating the form fields with existing data with ".initial"
    ...
    def save(self, commit=True):
        ...
        if self.instance and hasattr(self.instance, "organization"):
            ...
            organization.name = self.cleaned_data.get(
                "school_name"
            )  # this just checks if any of the prepopulated stuff has changed(i.e editing)
            ...
            organization.org_type = OrganizationType.objects.get(
                name="School"
            )  # we are setting the org type to school since its a school form
            organization.save()
        ...
        school = super().save(
            commit=False
        )  # saved without org info in memory not db , to avoid error,we are creating the instace and then adding to it
